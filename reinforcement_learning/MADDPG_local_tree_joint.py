...
from reinforcement_learning.maddpg_policy_joint import MADDPGPolicy
...
def train_agent(train_params, train_env_params, eval_env_params, obs_params):
    ...
    for i in range(n_agents):
        print("Agent {}: {}/{} experiences".format(i, len(policies[i].memory), train_params.buffer_size))
    ...
    for episode_idx in range(n_episodes + 1):
        ...
        for step in range(max_steps - 1):
            ...
            for agent in train_env.get_agent_handles():
                if info['action_required'][agent]:
                    ...
                    action_one_hot = policies[agent].act(agent_obs[agent][None, :], explore=True)[0]
        ...
        if episode_idx % checkpoint_interval == 0:
            for agent in train_env.get_agent_handles():
                torch.save(policies[agent].q, os.path.join(checkdir, '{}-{}-q.pth'.format(str(episode_idx), agent)))
                torch.save(policies[agent].p, os.path.join(checkdir, '{}-{}-p.pth'.format(str(episode_idx), agent)))

                if save_replay_buffer:
                    policies[i].save_replay_buffer(os.path.join(bufferdir, '{}-{}.pkl'.format(str(episode_idx), agent)))
        ...
        if episode_idx % checkpoint_interval == 0 and n_eval_episodes > 0:
            scores, completions, nb_steps_eval = eval_policy(eval_env, policies, train_params, obs_params)
...
def eval_policy(env, policies, train_params, obs_params):
    n_eval_episodes = train_params.n_evaluation_episodes
    ...
    for episode_idx in tqdm(range(n_eval_episodes)):
        ...
        for step in range(max_steps - 1):
            for agent in env.get_agent_handles():
                ...
                action = 0
                if info['action_required'][agent]:
                    action_one_hot = policies[agent].act(agent_obs[agent][None, :])[0]
                    action = np.argmax(action_one_hot.cpu().detach().numpy())
                action_dict.update({agent: action})

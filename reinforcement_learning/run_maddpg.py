...
from reinforcement_learning.maddpg_policy import MADDPGPolicy_LocalCritic as MADDPGPolicy
...
# MADDPG model path:
checkpoint = "train_log_final/maddpg_t0_e0/Jan20_19-48-58_ubuntu-imre_s20/checkpoints/4000-0-p.pth" 
# MADDPG+PS model path:
# checkpoint = "train_log_final/maddpg_ps_t0_e0/Jan20_13-12-36_Idiot_s0/checkpoints/4000-p.pth" 
# MADDPG_lc model path:
# checkpoint = "train_log_final/maddpg_t0_e0/Jan21_20-43-23_ubuntu-imre_s0/checkpoints/4000-0-p.pth" 
# MADDPG_lc+PS model path:
# checkpoint = "train_log_final/maddpg_ps_t0_e0/Jan21_17-07-51_Idiot_s0/checkpoints/4000-p.pth" 
...
policy = MADDPGPolicy(state_size, action_size, 1, 0, Namespace(**{'use_gpu': False}), evaluation_mode=True)

if os.path.isfile(checkpoint):
    policy.p = torch.load(checkpoint).to(policy.device)
else:
    raise ValueError("Checkpoint not found! (path: {})".format(checkpoint))
#####################################################################
                ...
                for agent in range(nb_agents):
                    if observation[agent] and info['action_required'][agent]:
                        if agent in agent_last_obs and np.all(agent_last_obs[agent] == observation[agent]):
                            ...
                            action_one_hot = policy.act(norm_obs[None, :])[0]
                            action = np.argmax(action_one_hot.cpu().detach().numpy())
